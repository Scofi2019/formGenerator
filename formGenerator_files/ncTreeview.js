	...
	this._setMaxWidth = function($node, refreshUI){
		...
		if($node && $node.length > 0){
			...
			maxWidth = (nodeLeft - left) + $node.outerWidth() - $node.width() +
			           checkWidth($arrow.outerWidth(true)) + checkWidth($check.outerWidth(true)) + 
			           checkWidth($icon.outerWidth(true)) + checkWidth($content.outerWidth(true));
			
			this._maxWidthMap[$node.attr("node-id")] = maxWidth;
			
			if(checkWidth(this._maxWidth) < maxWidth){
	...
	this.setHeight = function(height){
		if(height == "auto"){
		    ...
		    this.$view.find(".ncTreeviewBody").height(height - 
		    		this.$view.children(".ncTreeviewHead").outerHeight(true) - this.$view.children(".ncTreeviewSearch").outerHeight(true));
		}
	}
