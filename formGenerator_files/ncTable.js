...
function __ncTable(option){
		...
		if($items.length > 0){
			for(var i=0;i<$items.length;i++){
				...
				var col = {"name":$item.attr("name")?$item.attr("name"):"", 
					       ...
					       "editable":$item.attr("nc-editable"),
	...
	this.loadData = function(url, param, option){
		...
		$.ajax({
			...
			success : function(data) {
				...
				if(data){
					list = data.gridResult?data.gridResult:data;
					myself.loadLocalData(list);
					//设置分页数据
				...
				if(myself.option && myself.option.loadComplete){
					myself.option.loadComplete.call(myself, list);
				}
			}
		});
	}
    ...
    //载入本地数据
	this.loadLocalData = function(list){
		myself.beginUpdate();
	...
	//立即插入行
	this.insertRow = function(item){
		if(!this._cache_data || this._cache_data.length == 0){
			this.clear();
			this._cache_data = [];
		}
		this._cache_data.push(item);
		this._rowcount = this._rowcount == null?0:this._rowcount;
		this._ind = this._ind == null?0:this._ind;
		this.addRow(item);
		this.endUpdate({insertRow:true});
	}
	
	//删除行
	this.deleteRow = function(ind){
		var $r = this.$table.find(".ncTableBody:first>.ncTableRow[nc-ind='"+ind+"']");
		var rowid = $r.attr("nc-index");
		$r.remove();
		
		$r = this.$table.find(".ncTableRowHead:first>.ncTableRow[nc-ind='"+ind+"']");
		$r.remove();
		
		var shiftRow = function(selector, resetCacheData){
			var $rs = myself.$table.find(selector + ":first>.ncTableRow");
			var cd = [], htm = "";
			
			$rs.each(function(){
				var $this = $(this);
				var index = $this.attr("nc-index"),
				    cid = $this.attr("nc-ind");
				
				if(!index || ind == cid) return;
				
				index = Number(index);
				
				if(index > Number(rowid)){
					var $ari = $this.find(".ncTableBodyItem[nc-type='attrRowIndex']");
					$ari.text(index - 1);
					$this.attr("nc-index", index - 1);
				}
				
				htm += $this[0].outerHTML;
				
				if(resetCacheData){
					cd.push(myself._cache_data[index - 1]);
				}
			});
			
			if(resetCacheData){
				myself._cache_data = cd;
			}
			
			return htm;
		}
		
		myself._fixedColHTML = shiftRow(".ncTableRowHead");
		myself._cacheRowHTML = shiftRow(".ncTableBody", true);
		this._rowcount--;
	}
	...
	this.addRow = function(item){
		...
		this._fixedColHTML += '<div class="ncTableRow" nc-index={index} nc-ind={ind}>';
		...
		if(this._fixedCols && this._fixedCols.length > 0){
			...
			this._fixedColHTML = this._fixedColHTML.replace("{index}", this._rowcount)
			                                       .replace("{ind}", this._ind);
			for(var i=0;i<this._fixedCols.length;i++){
				var fixedCol = this._fixedCols[i];
				this._fixedColHTML += this._buildBodyItem(fixedCol, item, {sumWidth: this._fixedColWidth, rowid:this._rowcount, ind:this._ind});
			}
		}
		...
		this._cacheRowHTML += '<div class="ncTableRow" nc-index={index} nc-ind={ind} style="'+style.join(";")+'">';
        	...
        	for(var i=0;i<this._columns.length;i++){
        		...
        		if(col["class"] == "ncTableHeadItem fillCol"){
        			if(minus > 0){
        				col.width = minus;
        				col.hidden = false;
            			this._cacheRowHTML += this._buildBodyItem(col, item, {sumWidth: sw, rowid:this._rowcount, ind:this._ind});
        			}else{
        			}
        		}else{
        			this._cacheRowHTML += this._buildBodyItem(col, item, {sumWidth: sw, rowid:this._rowcount, ind:this._ind});
		...
		if(col.type){
			if(col.type == "custom"){
				...
				if(this.option.custom && this.option.custom[col.name]){
					html += this.option.custom[col.name].formatter.call(this, item[col.name], opt.rowid, item, opt.ind);
				}
    ...
    this.endUpdate = function(opt){
    	...
    	if(opt && opt.fail){
    		...
    		var top = (this.$table.height()-$fail.height())/2;
    		top = top < 0 ? 50 : top;
    		
        	$fail.css({"left":(this.$table.width()-$fail.width())/2+"px", "top":top+"px"});
    ...
    this._bindEvent = function(type){
    	if(type == "load"){
			...
			if(this.option.selectRowMode != "none"){
    		...
    		$rows.click(function(e){
				...
				var valueChanged = function(rowIndex, cellName, cellValue){
					if(!rowIndex) return;

                    myself._cache_data[rowIndex - 1][cellName] = cellValue;

					if(myself.option.event && myself.option.event.valueChanged){
						myself.option.event.valueChanged.call(myself, rowIndex, cellName, cellValue);
					}
				}
        		
        		
				var $items = $this.find(".ncTableBodyItem");
				var $lastInput = myself.$table.find(".ncTableCellInput"), delFlg = false;
				...
				for(var i=0;i<$items.length;i++){
					...
					if(e.pageX > $item.offset().left && e.pageX < $item.offset().left + myself._inMethods.getElementWidth($item) &&
					   e.pageY > $item.offset().top && e.pageY < $item.offset().top + myself._inMethods.getElementHeight($item)){
						...
						var cellName = $item.attr("name");

						if(myself.isEditable(cellName)){
							var $input = $item.find(".ncTableCellInput");
							if($input.length == 0){
								var t = $item.text().trim();

								$input = $("<input type='text' class='ncTableCellInput'/>");
								...
								$input.keydown(function(e){
									if(e.keyCode == 13){
										var $in = $(e.target), 
											val = $in.val(), 
											index = $in.parent().parent().attr("nc-index");

										$in.parent().text(val);
										$in.remove();

										valueChanged(index, cellName, val);
									}
								});
						...
						if(event.cellClick){
							event.cellClick.call(myself, myself.getCurrentRowData(), rowIndex, cellName, $item);
						}
					}
        		}
				...
				if(delFlg){
					var val = $lastInput.val(), 
						cellName = $lastInput.parent().attr("name"),
						index = $lastInput.parent().parent().attr("nc-index");

					$lastInput.parent().text(val);
					$lastInput.remove();
    ...
    this.getAllRowData = function(){
    	var rowData = [];
    	for(i in this._cache_data){
    		var item = this._cache_data[i], tmp = {};
    		for(var j in item){
    			tmp[j] = item[j] == null?"":item[j];
    		}
    		rowData.push(tmp);
    	}
    	return rowData;
	...
	this.isEditable = function(name){
		for(var i in this._columns){
			var col = this._columns[i];
			if(col.name == name && col.editable == "true"){
			    return true;
			}
		}
